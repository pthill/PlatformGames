...
int CGameClientView::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	//游戏框架
	m_ImageGameFrame[0].LoadImage(hResInstance,TEXT("FRAME_TL"));
	m_ImageGameFrame[1].LoadImage(hResInstance,TEXT("FRAME_TM"));
	m_ImageGameFrame[2].LoadImage(hResInstance,TEXT("FRAME_TR"));
	m_ImageGameFrame[3].LoadImage(hResInstance,TEXT("FRAME_LM"));
	m_ImageGameFrame[4].LoadImage(hResInstance,TEXT("FRAME_RM"));
	m_ImageGameFrame[5].LoadImage(hResInstance,TEXT("FRAME_BL"));
	m_ImageGameFrame[6].LoadImage(hResInstance,TEXT("FRAME_BM"));
	m_ImageGameFrame[7].LoadImage(hResInstance,TEXT("FRAME_BR")); 
...
VOID CGameClientView::RectifyControl(INT nWidth, INT nHeight)
{
	...
	int TopHeight = nHeight/2-iHeight/2+FRAME_HEIGHT;

	int LifeWidth1 = nWidth/2+iWidth/3-47;
	int TopHeight1 = nHeight/2+12+FRAME_HEIGHT;
	...
	DeferWindowPos(hDwp,m_btChip100,NULL,(nWidth-m_ImageChangeBack.GetWidth())/2+74,(nHeight-m_ImageChangeBack.GetHeight())/2+148+FRAME_HEIGHT,0,0,uFlags|SWP_NOSIZE);
	DeferWindowPos(hDwp,m_btChip1000,NULL,(nWidth-m_ImageChangeBack.GetWidth())/2+74*2+4,(nHeight-m_ImageChangeBack.GetHeight())/2+148+FRAME_HEIGHT,0,0,uFlags|SWP_NOSIZE);
	DeferWindowPos(hDwp,m_btChip10000,NULL,(nWidth-m_ImageChangeBack.GetWidth())/2+74*3+7,(nHeight-m_ImageChangeBack.GetHeight())/2+148+FRAME_HEIGHT,0,0,uFlags|SWP_NOSIZE);
	DeferWindowPos(hDwp,m_btChipAll,NULL,(nWidth-m_ImageChangeBack.GetWidth())/2+74*4+10,(nHeight-m_ImageChangeBack.GetHeight())/2+148+FRAME_HEIGHT,0,0,uFlags|SWP_NOSIZE);
	DeferWindowPos(hDwp,m_btChipClose,NULL,(nWidth-m_ImageChangeBack.GetWidth())/2+270,(nHeight-m_ImageChangeBack.GetHeight())/2+264+FRAME_HEIGHT,0,0,uFlags|SWP_NOSIZE);
	DeferWindowPos(hDwp,m_btChip_X,NULL,(nWidth+m_ImageChangeBack.GetWidth())/2-34,(nHeight-m_ImageChangeBack.GetHeight())/2+12+FRAME_HEIGHT,0,0,uFlags|SWP_NOSIZE);
	DeferWindowPos(hDwp,m_btChipOK,NULL,(nWidth-m_ImageChangeBack.GetWidth())/2+93,(nHeight-m_ImageChangeBack.GetHeight())/2+264+FRAME_HEIGHT,0,0,uFlags|SWP_NOSIZE);
...
{	
	...
	for ( int iW = 0 ; iW < nWidth; iW += m_ImageViewFill.GetWidth() )
	{
		...
		{
			m_ImageViewFill.BitBlt(pDC->GetSafeHdc(), iW, iH+FRAME_HEIGHT);
		}
	}
	m_ViewBackPng.DrawImage(pDCBuff,(nWidth-m_ViewBackPng.GetWidth())/2,nHeight/2-m_ViewBackPng.GetHeight()/2+10+FRAME_HEIGHT);
	...
	int nDestHeight = m_idb_selPng.GetHeight()+FRAME_HEIGHT;
	if(m_blRungingCar && cbGameStatus == GS_GAME_END)
	{
		m_idb_selPng.DrawImage(pDCBuff ,m_CarRect[m_CarIndex].left,m_CarRect[m_CarIndex].top+FRAME_HEIGHT,nDestWidth,nDestHeight,0,0);
		m_idb_selPng.DrawImage(pDCBuff ,m_CarRect[(m_CarIndex-1+24)%24].left,m_CarRect[(m_CarIndex-1+24)%24].top+FRAME_HEIGHT,nDestWidth,nDestHeight,nDestWidth,0);
		m_idb_selPng.DrawImage(pDCBuff ,m_CarRect[(m_CarIndex-2+24)%24].left,m_CarRect[(m_CarIndex-2+24)%24].top+FRAME_HEIGHT,nDestWidth,nDestHeight,nDestWidth*2,0);
		m_idb_selPng.DrawImage(pDCBuff ,m_CarRect[(m_CarIndex-3+24)%24].left,m_CarRect[(m_CarIndex-3+24)%24].top+FRAME_HEIGHT,nDestWidth,nDestHeight,nDestWidth*3,0);
	}
	if(m_bFlashrandShow)
	{
		m_idb_selPng.DrawImage(pDCBuff ,m_CarRect[m_CarIndex].left,m_CarRect[m_CarIndex].top+FRAME_HEIGHT,nDestWidth,nDestHeight,0,0);
	...
	if(m_bTip)
	{
		m_ImageTip.DrawImage(pDC,nWidth/2-m_ImageTip.GetWidth()/2+77,nHeight/2-m_ImageTip.GetHeight()/2-50+FRAME_HEIGHT,
	...
	if(m_wMeChairID!=INVALID_CHAIR && m_lExchangeScore[m_wMeChairID]>=0)
	{
		...
		DrawNumber(pDCBuff,&m_ImageScoreNumberPng,TEXT("0123456789"),m_lExchangeScore[m_wMeChairID],nWidth/2+170,nHeight/2-346+FRAME_HEIGHT,DT_RIGHT);
	}	
	...
	if(cbGameStatus==GS_COMPARE_END)
	{
		if(m_bGuess)
		{
			if(m_nGuessNum<10)
			{
				DrawNumberString(pDCBuff,m_nGuessNum,m_LifeWidth+329,nHeight/2+72+FRAME_HEIGHT,true);
			}
			else
			{
				DrawNumberString(pDCBuff,m_nGuessNum,m_LifeWidth+313,nHeight/2+72+FRAME_HEIGHT,true);
			}
		}
	}	
	...
	if(m_DrawBack)
	{
		...
		{
			if(wUserTimer<10)
				DrawNumberString(pDCBuff,wUserTimer,m_LifeWidth+329,nHeight/2+72+FRAME_HEIGHT,true);
			else
				DrawNumberString(pDCBuff,wUserTimer,m_LifeWidth+313,nHeight/2+72+FRAME_HEIGHT,true);
		}
	}	

	//胜利边框
	FlashJettonAreaFrame( nWidth, nHeight+FRAME_HEIGHT, pDCBuff);	
	...
	if(m_bEffect)
	{
		if(m_nProbability==2)
		{
			...
			m_EffectBigSanYuan.DrawImage(pDC,nWidth/2-183,nHeight/2-165+FRAME_HEIGHT,SizeFrame.cx,SizeFrame.cy,m_nEffectIndex*SizeFrame.cx,0);
		}
		else if(m_nProbability==3)
		{
			CSize SizeFrame(m_EffectSmallSanYuan.GetWidth()/5,m_EffectSmallSanYuan.GetHeight());
			m_EffectSmallSanYuan.DrawImage(pDC,nWidth/2-183,nHeight/2-165+FRAME_HEIGHT,SizeFrame.cx,SizeFrame.cy,m_nEffectIndex*SizeFrame.cx,0);
		}
		else if(m_nProbability==4)
		{
			CSize SizeFrame(m_EffectBigSiXi.GetWidth()/5,m_EffectBigSiXi.GetHeight());
			m_EffectBigSiXi.DrawImage(pDC,nWidth/2-183,nHeight/2-165+FRAME_HEIGHT,SizeFrame.cx,SizeFrame.cy,m_nEffectIndex*SizeFrame.cx,0);
		}
		else if(m_nProbability==5)//通吃
		{			
			m_ImageEat.DrawImage(pDC,nWidth/2-183,nHeight/2-165+FRAME_HEIGHT);
		}
		else
		{
			//播放动画
			PlayEffect(pDCBuff,nWidth,nHeight+FRAME_HEIGHT);
		}		
	}
	if(m_blMoveFinish&&(cbGameStatus==GS_GAME_END||cbGameStatus==GS_COMPARE_END))
	{
		...
		{			
			DrawNumber(pDCBuff,&m_ImageScoreNumberPng,TEXT("0123456789"),m_lMeCurGameReturnScore[m_wMeChairID],nWidth/2-100,nHeight/2-346+FRAME_HEIGHT,DT_RIGHT);
		}
	}	
	...
	if(m_DrawBack)
	{
		DrawBankerInfo(pDCBuff,nWidth,nHeight+FRAME_HEIGHT);		
	}

	//绘画上庄列表
	DrawBankerList(pDC,nWidth,nHeight+FRAME_HEIGHT);
	...
	if(cbGameStatus!=GS_GAME_FREE)
	{
		...
		{
			TCHAR Text[256];
			_sntprintf(Text,256,TEXT("%I64d/%I64d"),m_lAllJettonScore[i+1], m_lBankerScore);
	...
	//画倍数框
	if(m_nTime==1) m_ImageTimesFrame1.DrawImage(pDC,nWidth/2-292+64*8,nHeight/2+231+FRAME_HEIGHT);
	else if(m_nTime==10) m_ImageTimesFrame1.DrawImage(pDC,nWidth/2-292+64*8+50,nHeight/2+231+FRAME_HEIGHT);
	else if(m_nTime==100) m_ImageTimesFrame2.DrawImage(pDC,nWidth/2-287+64*8+95,nHeight/2+231+FRAME_HEIGHT);
	...
	if(m_bShow)
	{
		...
		m_ImageChangeBack.DrawImage(pDCBuff,nWidth/2-m_ImageChangeBack.GetWidth()/2,nHeight/2-m_ImageChangeBack.GetHeight()/2+FRAME_HEIGHT);
		...
		_sntprintf( szBuffer, CountArray(szBuffer), TEXT("%s"), AddDecimal(m_lUserScore) );
		DrawDecimal(pDCBuff,&m_ImageChipNum,TEXT("+-,.0123456789"), szBuffer, nWidth/2-m_ImageChangeBack.GetWidth()/2+238,nHeight/2-m_ImageChangeBack.GetHeight()/2+98+FRAME_HEIGHT);

		//DrawNumberStringEX(pDCBuff,1,nWidth/2-m_ImageChangeBack.GetWidth()/2+190,nHeight/2-m_ImageChangeBack.GetHeight()/2+208,true);
		DrawNumber(pDCBuff,&m_ImageChipNum,TEXT("+-,.0123456789"),1,nWidth/2-m_ImageChangeBack.GetWidth()/2+150,nHeight/2-m_ImageChangeBack.GetHeight()/2+198+FRAME_HEIGHT);
		...
		DrawNumber(pDCBuff,&m_ImageChipNum,TEXT("+-,.0123456789"),m_nExchange,nWidth/2-m_ImageChangeBack.GetWidth()/2+279,nHeight/2-m_ImageChangeBack.GetHeight()/2+198+FRAME_HEIGHT);
		...
		rcChip.top = nHeight/2-m_ImageChangeBack.GetHeight()/2+224+FRAME_HEIGHT;
		rcChip.bottom = rcChip.top+15;
		TCHAR szChipNum[32];
		_sntprintf(szChipNum,32,TEXT("%s"), AddDecimal(m_lCanExchangeScore) );
...
void CGameClientView::DrawGameFrame(CDC *pDC, int nWidth, int nHeight)
{
	...
	m_FontCaption.DrawText( pDC, m_StringCaption, rcCaption, RGB(255,255,0), DT_VCENTER|DT_LEFT);
}
...
// 绘画小数
void CGameClientView::DrawDecimal(CDC * pDC, CPngImage* ImageNumber, TCHAR * szImageNum, TCHAR* szOutNum ,INT nXPos, INT nYPos,  UINT uFormat)
{
	// 加载资源
	INT nNumberHeight=ImageNumber->GetHeight();
	INT nNumberWidth=ImageNumber->GetWidth()/lstrlen(szImageNum);
...
void CGameClientView::DrawBankerInfo(CDC *pDC,int nWidth,int nHeight)
{
	...
	StrRect.top = nHeight/2-m_ViewBackPng.GetHeight()/2 + 96 + FRAME_HEIGHT - 6;
