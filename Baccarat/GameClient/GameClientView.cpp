...
//按钮标识
#define IDC_CM_BUTTON_100			201									//按钮标识
#define IDC_CM_BUTTON_1000			202									//按钮标识
#define IDC_CM_BUTTON_2000			203									//按钮标识
#define IDC_CM_BUTTON_5000			204									//按钮标识
#define IDC_CM_BUTTON_10000			205									//按钮标识
#define IDC_CM_BUTTON_50000			206									//按钮标识
...
BEGIN_MESSAGE_MAP(CGameClientView, CGameFrameViewGDI)
	...
	ON_BN_CLICKED(IDC_CM_BUTTON_2000, OnBetButton2000)
	ON_BN_CLICKED(IDC_CM_BUTTON_5000, OnBetButton5000)
...
int CGameClientView::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	m_btBet2000.Create(NULL,WS_CHILD|WS_VISIBLE|WS_DISABLED,rcCreate,this,IDC_CM_BUTTON_2000);
	m_btBet5000.Create(NULL,WS_CHILD|WS_VISIBLE|WS_DISABLED,rcCreate,this,IDC_CM_BUTTON_5000);
	m_btBet10000.Create(NULL,WS_CHILD|WS_VISIBLE|WS_DISABLED,rcCreate,this,IDC_CM_BUTTON_10000);
	m_btBet50000.Create(NULL,WS_CHILD|WS_VISIBLE|WS_DISABLED,rcCreate,this,IDC_CM_BUTTON_50000);
	...
	m_btBet100.SetButtonImage(BT_CM_100,hResInstance,false,false);
	m_btBet1000.SetButtonImage(BT_CM_1000,hResInstance,false,false);
	m_btBet2000.SetButtonImage(BT_CM_2000,hResInstance,false,false);
	m_btBet5000.SetButtonImage(BT_CM_5000,hResInstance,false,false);
	m_btBet10000.SetButtonImage(BT_CM_10000,hResInstance,false,false);
	m_btBet50000.SetButtonImage(BT_CM_50000,hResInstance,false,false);
...
VOID CGameClientView::RectifyControl(int nWidth, int nHeight)
{
	...
	DeferWindowPos(hDwp,m_btBet100,NULL,nWidth/2 - 320 +7,470 ,0,0,uFlags|SWP_NOSIZE);
	DeferWindowPos(hDwp,m_btBet1000,NULL,nWidth/2 - 206,511,0,0,uFlags|SWP_NOSIZE);
	DeferWindowPos(hDwp,m_btBet2000,NULL,nWidth/2 - 96,524,0,0,uFlags|SWP_NOSIZE);
	DeferWindowPos(hDwp,m_btBet5000,NULL,nWidth/2 + 96 - 89,524,0,0,uFlags|SWP_NOSIZE);
	DeferWindowPos(hDwp,m_btBet10000,NULL,nWidth/2 + 206 - 89-1,512,0,0,uFlags|SWP_NOSIZE);
	DeferWindowPos(hDwp,m_btBet50000,NULL,nWidth/2 + 320 - 89 - 5,470,0,0,uFlags|SWP_NOSIZE);
...
void CGameClientView::DrawGameView(CDC * pDC, int nWidth, int nHeight)
{
	...
	for (INT i=0;i<8;i++)
	{
		...
		LONGLONG lScoreBet[BET_COUNT]={100L,1000L,2000L,5000L,10000L,50000L};
		...
		{
			TCHAR szScore[128] = {0};
			LONGLONG dScoreCount = lScoreCount/100;
			_sntprintf(szScore,CountArray(szScore),TEXT("%s"), AddComma(dScoreCount));
	...
	for ( int i = 0; i < AREA_MAX; ++i )
	{
		...
		LONGLONG dPlayBet = m_lPlayBet[i]/100;
		...
		DrawNumber(pDC,&m_ImageMeScoreNumber, TEXT(",0123456789"), AddComma(dPlayBet),m_ptBetNumber[i].x,m_ptBetNumber[i].y - m_ImageMeScoreNumber.GetHeight()/2, DT_CENTER);
	}
...
void CGameClientView::DrawTopInfo(CDC *pDC, int nWidth, int nHeight)
{
	...
	{ 
		...
		DrawTextString( pDC, AddDecimal(GetMaxPlayerScore(AREA_ZHUANG)), RGB(78,220,224), RGB(39,27,1), ptBegin.x - 205, ptBegin.y);

		DrawTextString( pDC, AddDecimal(GetMaxPlayerScore(AREA_XIAN)), RGB(112,223,118), RGB(39,27,1), ptBegin.x - 18, ptBegin.y);

		DrawTextString( pDC, AddDecimal(GetMaxPlayerScore(AREA_PING)), RGB(253,111,109), RGB(39,27,1), ptBegin.x + 170, ptBegin.y);
	}
	...
	while ( nIndex != m_nRecordLast && ( m_nRecordLast!=m_nRecordFirst ) && nDrawCount < 12 )
	{
		...
		DrawTextString( pDC, AddDecimal(ClientGameRecord.cbBankerCount), RGB(205,173,80), RGB(39,27,1), ptBegin.x + 39 + nDrawCount * 15, ptBegin.y + 83);

		DrawTextString( pDC, AddDecimal(ClientGameRecord.cbPlayerCount), RGB(205,173,80), RGB(39,27,1), ptBegin.x + 39 + nDrawCount * 15, ptBegin.y + 103);
	...
	DrawTextString( pDC, AddDecimal(lBankerScore), RGB(255,223,44), RGB(0,31,18), rcBanker, DT_END_ELLIPSIS|DT_LEFT|DT_VCENTER|DT_SINGLELINE);
	
	// 庄家成绩
	rcBanker.OffsetRect(0, 21);
	DrawTextString( pDC, AddDecimal(m_lBankerWinScore), RGB(254,110,110), RGB(0,31,18), rcBanker, DT_END_ELLIPSIS|DT_LEFT|DT_VCENTER|DT_SINGLELINE);
	
	// 庄家局数
	rcBanker.OffsetRect(0, 21);
	TCHAR szBankerTime[16]=TEXT("");
	_sntprintf(szBankerTime,CountArray(szBankerTime), TEXT("%d"), m_wBankerTime );
	DrawTextString( pDC, szBankerTime, RGB(106,192,217), RGB(0,31,18), rcBanker, DT_END_ELLIPSIS|DT_LEFT|DT_VCENTER|DT_SINGLELINE);
	...
	if (m_wMeChairID != INVALID_CHAIR)
	{
		...
		if ( pMeUserItem != NULL )
		{
			...
			DrawTextString(pDC, &m_FontValleysListTwo, AddDecimal( pMeUserItem->GetUserScore() - lMeBet ), RGB(255,225,45), RGB(34,26,3), rect, DT_END_ELLIPSIS|DT_LEFT|DT_VCENTER|DT_SINGLELINE);

			// 成绩
			rect.OffsetRect(0, 21);
			DrawTextString(pDC, &m_FontValleysListTwo, AddDecimal(m_lMeStatisticScore), RGB(83,204,101), RGB(34,26,3), rect, DT_END_ELLIPSIS|DT_LEFT|DT_VCENTER|DT_SINGLELINE);
	...
	for( int i = m_nShowValleyIndex; i < m_ValleysList.GetCount(); ++i )
	{
		...
		DrawTextString( pDC,AddDecimal(pIClientUserItem->GetUserScore() ), RGB(255,249,102) , RGB(0,0,0), rectScore, DT_END_ELLIPSIS|DT_LEFT|DT_VCENTER|DT_SINGLELINE);
...
void CGameClientView::DrawBottomInfo(CDC *pDC, int nWidth, int nHeight)
{
	...
	DrawTextString( pDC, AddDecimal(nBankerCount), RGB(205,173,80), RGB(39,27,1), rect, DT_END_ELLIPSIS|DT_CENTER|DT_VCENTER|DT_SINGLELINE);
	
	rect.OffsetRect(0, 26);
	rect.bottom += 1;
	DrawTextString( pDC, AddDecimal(nPlayerCount), RGB(205,173,80), RGB(39,27,1), rect, DT_END_ELLIPSIS|DT_CENTER|DT_VCENTER|DT_SINGLELINE);
	
	rect.OffsetRect(0, 28);
	DrawTextString( pDC, AddDecimal(nTieCount), RGB(205,173,80), RGB(39,27,1), rect, DT_END_ELLIPSIS|DT_CENTER|DT_VCENTER|DT_SINGLELINE);*/
...
void CGameClientView::DrawGameOver(CDC *pDC, int nWidth, int nHeight, int nBeginX, int nBeginY )
{
	...
	// 分数
	DrawNumber(pDC, &m_ImageGameEndMyScore, TEXT("+-,0123456789"), AddComma(m_lPlayScore[AREA_XIAN_DUI]/100, true), nBeginX + 87 , nBeginY + 125, DT_CENTER);
	DrawNumber(pDC, &m_ImageGameEndMyScore, TEXT("+-,0123456789"), AddComma(m_lPlayScore[AREA_ZHUANG_DUI]/100, true), nBeginX + 334 , nBeginY + 125, DT_CENTER);

	DrawNumber(pDC, &m_ImageGameEndMyScore, TEXT("+-,0123456789"), AddComma(m_lPlayScore[AREA_XIAN]/100, true), nBeginX + 87 , nBeginY + 174, DT_CENTER);
	DrawNumber(pDC, &m_ImageGameEndMyScore, TEXT("+-,0123456789"), AddComma(m_lPlayScore[AREA_PING]/100, true), nBeginX + 208 , nBeginY + 174, DT_CENTER);
	DrawNumber(pDC, &m_ImageGameEndMyScore, TEXT("+-,0123456789"), AddComma(m_lPlayScore[AREA_ZHUANG]/100, true), nBeginX + 334 , nBeginY + 174, DT_CENTER);

	DrawNumber(pDC, &m_ImageGameEndMyScore, TEXT("+-,0123456789"), AddComma(m_lPlayScore[AREA_XIAN_TIAN]/100, true), nBeginX + 87 , nBeginY + 223, DT_CENTER);
	DrawNumber(pDC, &m_ImageGameEndMyScore, TEXT("+-,0123456789"), AddComma(m_lPlayScore[AREA_TONG_DUI]/100, true), nBeginX + 208 , nBeginY + 223, DT_CENTER);
	DrawNumber(pDC, &m_ImageGameEndMyScore, TEXT("+-,0123456789"), AddComma(m_lPlayScore[AREA_ZHUANG_TIAN]/100, true), nBeginX + 334 , nBeginY + 223, DT_CENTER);
	...
	DrawNumber(pDC, &m_ImageGameEndAllScore, TEXT("+-,0123456789"), AddComma(m_lPlayAllScore/100, true), nBeginX + 97 , nBeginY + 259, DT_LEFT);
	//DrawNumber(pDC, &m_ImageGameEndPoint, TEXT("0123456789"), AddDecimal(m_lRevenue, true), nBeginX + 97 , nBeginY + 293, DT_LEFT);
}
...
void CGameClientView::AddChip(BYTE cbViewIndex, LONGLONG lScoreCount)
{
	...
	//变量定义
	LONGLONG lScoreIndex[BET_COUNT]={100L,1000L,2000L,5000L,10000L,50000L};
...
//筹码按钮
void CGameClientView::OnBetButton2000()
{
	//设置变量
	m_lCurrentBet=2000L;
...
//筹码按钮
void CGameClientView::OnBetButton5000()
{
	//设置变量
	m_lCurrentBet=5000L;
...
BOOL CGameClientView::OnSetCursor(CWnd * pWnd, UINT nHitTest, UINT uMessage)
{
	if ( m_lCurrentBet != 0L && m_cbGameStatus == GAME_SCENE_BET )
	{
		...
		case 2000:
			{
				SetCursor( LoadCursor(AfxGetInstanceHandle(),MAKEINTRESOURCE(IDC_SCORE_2000)) );
				return TRUE;
			}
		case 5000:
			{
				SetCursor( LoadCursor(AfxGetInstanceHandle(),MAKEINTRESOURCE(IDC_SCORE_5000)) );
				return TRUE;
			}
		case 10000:
			{
...
void CGameClientView::InsertGameEndInfo()
{
	...
	strInfo.Format(TEXT("庄家:%s"), AddDecimal( m_lBankerCurGameScore) );

	InsertAllChatMessage(strInfo,0);

	strInfo.Format(TEXT("本家:%s"), AddDecimal( m_lPlayAllScore) );
...
void CGameClientView::DeduceWinner(BYTE &cbWinner, BYTE &cbKingWinner, bool &bPlayerTwoPair, bool &bBankerTwoPair)
{
	...
	if (cbPlayerCount == cbBankerCount)
	{
		...
		{
			WORD wCardIndex = 0;
			for (wCardIndex; wCardIndex < m_cbCardCount[INDEX_PLAYER]; ++wCardIndex )
			{
				BYTE cbBankerValue = m_GameLogic.GetCardValue(m_cbTableCardArray[INDEX_BANKER][wCardIndex]);
				BYTE cbPlayerValue = m_GameLogic.GetCardValue(m_cbTableCardArray[INDEX_PLAYER][wCardIndex]);
				if ( cbBankerValue != cbPlayerValue ) break;
...
// 添加逗号
CString CGameClientView::AddDecimal( LONGLONG lScore , bool bPlus /*= false*/)
{
	CString strScore;
	CString strReturn;
	LONGLONG lNumber = lScore;
...
void CGameClientView::OnSize( UINT nType, int cx, int cy )
{
	HDWP hDwp=BeginDeferWindowPos(32);
	const UINT uFlags=SWP_NOACTIVATE|SWP_NOZORDER|SWP_NOCOPYBITS;
	//DeferWindowPos(hDwp,m_btBet100,NULL,197-3,470-4,0,0,uFlags|SWP_NOSIZE);
	//m_btBet100.MoveWindow(100,100,300,300,TRUE);
}
