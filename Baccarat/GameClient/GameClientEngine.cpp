...
bool CGameClientEngine::OnEventGameClockInfo(WORD wChairID, UINT nElapse, WORD wClockID)
{
	if ((wClockID == IDI_PLACE_JETTON)&&(nElapse == 0))
	{
		...
		m_GameClientView.m_btBet2000.EnableWindow(FALSE);
		m_GameClientView.m_btBet5000.EnableWindow(FALSE);
		m_GameClientView.m_btBet10000.EnableWindow(FALSE);
		m_GameClientView.m_btBet50000.EnableWindow(FALSE);
...
void CGameClientEngine::UpdateButtonContron()
{
	...
	if ( bEnablePlaceBet )
	{
		...
		{
			if (lLeaveScore>=50000L) m_GameClientView.SetCurrentBet(50000L);
			else if (lLeaveScore>=10000L) m_GameClientView.SetCurrentBet(10000L);
			else if (lLeaveScore>=5000L) m_GameClientView.SetCurrentBet(5000L);
			else if (lLeaveScore>=2000L) m_GameClientView.SetCurrentBet(2000L);
		...
		m_GameClientView.m_btBet2000.EnableWindow((lLeaveScore>=2000)?TRUE:FALSE);
		m_GameClientView.m_btBet5000.EnableWindow((lLeaveScore>=5000)?TRUE:FALSE);		
		m_GameClientView.m_btBet10000.EnableWindow((lLeaveScore>=10000)?TRUE:FALSE);
		m_GameClientView.m_btBet50000.EnableWindow((lLeaveScore>=50000)?TRUE:FALSE);
	}
	else
	{
		//设置光标
