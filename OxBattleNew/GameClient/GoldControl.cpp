...
{
	LONGLONG lGold=0L;
	for (LONGLONG i=0;i<CountArray(m_lGoldCell);i++) lGold+=m_lGoldCell[i]*(LONGLONG)pow(10.0,(int)i);
	return lGold;
}
...
void CGoldControl::OnLButtonUp(UINT nFlags, CPoint point)
{
	...
	{
		LONGLONG lAddGold=(LONGLONG)pow(10.0,iCellPos);
...
void CGoldControl::OnRButtonUp(UINT nFlags, CPoint point)
{
	...
	{
		LONGLONG lAddGold=9L*(LONGLONG)pow(10.0,iCellPos);
		if ((GetGold()+lAddGold)>m_lMaxGold) return;
	}
