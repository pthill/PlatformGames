...
VOID CScoreControl::OnEventDrawWindow(CD3DDevice * pD3DDevice, INT nXOriginPos, INT nYOriginPos)
{
	...
	for (WORD i=0;i<GAME_PLAYER;i++)
	{
		...
		_sntprintf(czGameScore,CountArray(czGameScore),TEXT("得分：%s"), AddDecimal(m_ScoreInfo.lGameScore[i]) );
		_sntprintf(czCollectScore,CountArray(czCollectScore),TEXT("累计：%s"), AddDecimal(m_ScoreInfo.lCollectScore[i]) );
...
// 添加逗号
CString CScoreControl::AddDecimal( LONGLONG lScore ,  bool bComma /*= true*/, bool bPlus /*= false*/)
{
	CString strScore;
	CString strReturn;
