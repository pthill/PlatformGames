...
VOID CGameClientView::DrawGameView(CD3DDevice * pD3DDevice, INT nWidth, INT nHeight)
{
	...
	if ( m_cbBankerScore != 0 && m_cbBankerScore != 255 )
	{
		...
		DrawNumber(pD3DDevice,&m_TextureCellNumber, TEXT("+-,.0123456789"),szOutNumT,(nWidth)/2 -42 ,136+27-30, DT_LEFT);
	}
	...
	for (WORD i=0;i<GAME_PLAYER;i++)
	{
		...
		CString strScore = AddDecimal(pIClientUserItem->GetUserScore());
