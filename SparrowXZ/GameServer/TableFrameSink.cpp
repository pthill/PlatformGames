...
bool CTableFrameSink::OnEventGameStart()
{
	...
	//	0x11,0x12,0x13,0x14,0x15,0x16,0x17,0x18,0x19,0x21,0x21,
	//	0x02,0x02,0x02,0x03,0x04,0x06,0x05,0x05,0x11,0x21,0x21,0x21,0x21,
	//	0x02,0x02,0x02,0x02,0x04,0x06,0x05,0x05,0x11,0x21,0x21,0x21,0x21,
	//	0x02,0x02,0x02,0x02,0x04,0x06,0x05,0x05,0x11,0x21,0x21,0x21,0x21,
	//	0x02,0x02,0x02,0x02,0x04,0x06,0x05,0x05,0x11,0x21,0x21,0x21,0x21,
	//};
	//CopyMemory(m_cbRepertoryCard,cbTempDebug,sizeof(BYTE)*108);
	////TEST+
	...
	{
		//计算数目
		BYTE cbValidCount=HEAP_FULL_COUNT-m_cbHeapCardInfo[wTakeChairID][1]-((i==0)?(cbSamllSiceTakeCount)*2:0);
		...
		if (cbTakeCount==0)
		{
			...
			m_wHeapTail=(m_wBankerUser+cbSamllSiceTakeCount-1)%GAME_PLAYER;
			//牌头加上4张,凑够108张牌
			if (m_cbHeapCardInfo[m_wHeapHand][0] < 4)
			{
				WORD wHeapHead = 4 - m_cbHeapCardInfo[m_wHeapHand][0];
				m_cbHeapCardInfo[m_wHeapHand][0] = 0;
				m_wHeapHand=(m_wHeapHand+GAME_PLAYER-1)%GAME_PLAYER;
				m_cbHeapCardInfo[m_wHeapHand][0] -= wHeapHead;
			}
			else m_cbHeapCardInfo[m_wHeapHand][0] -= 4;
					...
					if (bHuaZhu)
					{
						...
						for(WORD j = 0; j < GAME_PLAYER; j++)
						{
							...
							if (!m_GameLogic.IsHuaZhu(m_cbCardIndex[j],m_WeaveItemArray[j],m_cbWeaveItemCount[j],m_cbCallCard[j]))
							{
								...
								lHuaZhuScoreEx[i][j] = m_pGameServiceOption->lCellScore*16L;
								lLostHuaZhuScore[j][i] = m_pGameServiceOption->lCellScore*16L;
							}
						}
					}
								...
								if( m_GameLogic.IsTingCard(m_cbCardIndex[j],m_WeaveItemArray[j],m_cbWeaveItemCount[j],m_cbCallCard[j]) )
								{
									...
									for( BYTE k = 0; k < MAX_INDEX-7; k++ )
									{
										...
										if( WIK_CHI_HU == m_GameLogic.AnalyseChiHuCard(m_cbCardIndex[j],m_WeaveItemArray[j],
											m_cbWeaveItemCount[j],cbCurrentCard,chr,m_cbCallCard[j]) )
										{
											FiltrateRight(j,chr);
									...
									lChaJiaoScoreEx[i][j] = m_pGameServiceOption->lCellScore*wFanShu;
									lLostChaJiaoScore[j][i] = m_pGameServiceOption->lCellScore*wFanShu;
								}
							}
						}
					}
	...
	case GS_MJ_FREE:	//空闲状态
		{
			...
			StatusFree.bCheatMode=CServerRule::IsAllowAvertCheatMode(m_pGameServiceOption->dwServerRule);
	...
	case GS_MJ_CALL:	//缺门状态
		{
			...
			//堆立信息
			StatusCall.wHeapHand = m_wHeapHand;
...
{
	//效验状态
	ASSERT(m_pITableFrame->GetGameStatus() == GS_MJ_CALL);
	if (m_pITableFrame->GetGameStatus() != GS_MJ_CALL) return false;
	...
	for (WORD i=0; i<GAME_PLAYER; i++)
	{
		if (m_bCallCard[i] == true) cbCallCount++;
	}
...
bool CTableFrameSink::SendOperateNotify()
{
	...
	for (WORD i=0;i<GAME_PLAYER;i++)
	{
		...
		{
			m_pITableFrame->SendTableData(i,SUB_S_WAIT_OPERATE,NULL,0);
			m_pITableFrame->SendLookonData(i,SUB_S_WAIT_OPERATE,NULL,0);
		}
	}
...
void CTableFrameSink::ProcessChiHuUser( WORD wChairId, bool bGiveUp )
{
	if( !bGiveUp )
	{
		...
		//自摸番数
		if (m_wProvideUser == wChairId) wFanShu *= 2;
		...
		if( m_wProvideUser == wChairId )
		{
			for( WORD i = 0; i < GAME_PLAYER; i++ )
			{
				...
				if (m_bGangStatus==true && m_wProvideGang!=INVALID_CHAIR)
				{
					//胡牌分
					m_lGameScore[wChairId] += lChiHuScore;
					m_lGameScore[m_wProvideGang] -= lChiHuScore;
				}
				else
				{
					//胡牌分
		...
		if (m_cbWinCount==1) m_wFirstWinUser=wChairId;
		m_bGangStatus = false;
		m_bGangOutStatus = false;
		m_wProvideGang = INVALID_CHAIR;
