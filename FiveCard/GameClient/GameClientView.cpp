...
{
	TCHAR szOutNum[128] = {0};
	_sntprintf(szOutNum,CountArray(szOutNum),TEXT("%s"), AddDecimal(lOutNum,false,false) );
...
void CGameClientView::DrawOverScore( CDC * pDC, LONGLONG lOutNum, INT nXPos, INT nYPos, bool bMy, bool bAdd )
{
	...
	//分数转化
	LONGLONG lastOutNum = lOutNum;

	TCHAR szOutNum[128] = {0};
	TCHAR szImageNum[128] = { TEXT("+-,.0123456789") };
	if(bAdd)
	{
		_sntprintf(szOutNum,CountArray(szOutNum),TEXT("%s%s"),lastOutNum > 0 ? TEXT("+") : TEXT(""), AddDecimal(lastOutNum,false,false) );
	}
	else
	{
		_sntprintf(szOutNum,CountArray(szOutNum),TEXT("%s"),AddDecimal(lastOutNum,false,false));
	}

	INT nNumberHeight = m_ImgaeScoreNumber.GetHeight();
	INT nNumberWidth  = m_ImgaeScoreNumber.GetWidth() / 14;
