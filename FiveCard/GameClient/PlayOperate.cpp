...
void CPlayOperate::DrawPlayOperate( CDC * pDC, INT nWidth, INT nHeight )
{
	...
	if(m_bShowAdd)
	{
		...
		DrawNumber(pDC, &m_ImageScoreNumber, TEXT("+-,.0123456789"), AddDecimal(m_lScore), m_ptPos.x + 67, m_ptPos.y + 17, DT_CENTER, DT_TOP);
...
void CPlayOperate::SetCellScore( LONG lCellScore )
{
	...
	CString str;
	str.Format( TEXT("%s"), TEXT("X1") );
	m_btOneScore.SetWindowText(str);

	str.Format( TEXT("%s"), TEXT("X2") );
	m_btTwoScore.SetWindowText(str);

	str.Format( TEXT("%s"), TEXT("X3") );
	m_btThreeScore.SetWindowText(str);

	str.Format(TEXT("%s"),  TEXT("X4") );
...
// 添加逗号
CString CPlayOperate::AddDecimal( LONGLONG lScore ,  bool bComma /*= true*/, bool bPlus /*= false*/)
{
	CString strScore;
	CString strReturn;
