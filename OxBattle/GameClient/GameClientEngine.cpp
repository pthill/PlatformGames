	...
	case IDI_PLACE_JETTON:
		{
			if(nElapse == 0)
			{
				...
				//禁止按钮
				m_GameClientView.m_btJetton1.EnableWindow(FALSE);		
				m_GameClientView.m_btJetton5.EnableWindow(FALSE);		
				m_GameClientView.m_btJetton25.EnableWindow(FALSE);	
...
void CGameClientEngine::UpdateButtonContron()
{
	...
	if (bEnablePlaceJetton==true)
	{
		...
		{
			if (lLeaveScore>=1000000L) m_GameClientView.SetCurrentJetton(1000000L);
			...
			else if (lLeaveScore>=2500L) m_GameClientView.SetCurrentJetton(2500L);
		...
		//控制按钮
		int iTimer = 1;
		m_GameClientView.m_btJetton1.EnableWindow((lLeaveScore>=100*iTimer && lUserMaxJetton>=100*iTimer)?TRUE:FALSE);
		m_GameClientView.m_btJetton5.EnableWindow((lLeaveScore>=500*iTimer && lUserMaxJetton>=500*iTimer)?TRUE:FALSE);
		m_GameClientView.m_btJetton25.EnableWindow((lLeaveScore>=2500*iTimer && lUserMaxJetton>=2500*iTimer)?TRUE:FALSE);		
		m_GameClientView.m_btJetton100.EnableWindow((lLeaveScore>=10000*iTimer && lUserMaxJetton>=10000*iTimer)?TRUE:FALSE);	
		m_GameClientView.m_btJetton500.EnableWindow((lLeaveScore>=50000*iTimer && lUserMaxJetton>=50000*iTimer)?TRUE:FALSE);		
		m_GameClientView.m_btJetton1000.EnableWindow((lLeaveScore>=100000*iTimer && lUserMaxJetton>=100000*iTimer)?TRUE:FALSE);
		m_GameClientView.m_btJetton10000.EnableWindow((lLeaveScore>=1000000*iTimer && lUserMaxJetton>=1000000*iTimer)?TRUE:FALSE);
	}
	else
	{
