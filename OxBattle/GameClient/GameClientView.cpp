...
#define IDC_JETTON_BUTTON_1			290									//按钮标识
#define IDC_JETTON_BUTTON_5			291									//按钮标识
#define IDC_JETTON_BUTTON_25		292									//按钮标识
#define IDC_JETTON_BUTTON_100		293									//按钮标识
#define IDC_JETTON_BUTTON_500		294									//按钮标识
#define IDC_JETTON_BUTTON_1000		295									//按钮标识
#define IDC_JETTON_BUTTON_10000		296									//按钮标识
...
//位置信息
	...
	DeferWindowPos(hDwp,m_btJetton1,NULL,nWidth/2 - rcJetton.Width()/2 - 6*3 - rcJetton.Width()*3 +10,nYPos,0,0,uFlags|SWP_NOSIZE);
	DeferWindowPos(hDwp,m_btJetton5,NULL,nWidth/2 - rcJetton.Width()/2 - 6*2 - rcJetton.Width()*2 +10,nYPos,0,0,uFlags|SWP_NOSIZE);
	DeferWindowPos(hDwp,m_btJetton25,NULL,nWidth/2 - rcJetton.Width()/2 - 6 - rcJetton.Width() +10 ,nYPos,0,0,uFlags|SWP_NOSIZE);
	DeferWindowPos(hDwp,m_btJetton100,NULL,nWidth/2 - rcJetton.Width()/2 +10,nYPos,0,0,uFlags|SWP_NOSIZE);
	DeferWindowPos(hDwp,m_btJetton500,NULL,nWidth/2 + rcJetton.Width()/2 +6 +10,nYPos,0,0,uFlags|SWP_NOSIZE);
	DeferWindowPos(hDwp,m_btJetton1000,NULL,nWidth/2 + rcJetton.Width()/2 +6*2 + rcJetton.Width() +10,nYPos,0,0,uFlags|SWP_NOSIZE);
	DeferWindowPos(hDwp,m_btJetton10000,NULL,nWidth/2 + rcJetton.Width()/2 +6*3 + rcJetton.Width()*2 +10,nYPos,0,0,uFlags|SWP_NOSIZE);
...
// 			break;
	...
	case IDC_AUTO_OPEN_CARD:
			OnAutoOpenCard();
			break;
	case IDC_JETTON_BUTTON_1:
		{
			//设置变量
			m_lCurrentJetton=100L;
			break;
		}
	case IDC_JETTON_BUTTON_5:
		{
			//设置变量
			m_lCurrentJetton=500L;
			break;
		}
	case IDC_JETTON_BUTTON_25:
		{
			//设置变量
			m_lCurrentJetton=2500L;
			break;
		}
...
VOID CGameClientView::DrawGameView(CDC * pDC, INT nWidth, INT nHeight)
{
	...
	for (INT i=0;i<AREA_COUNT;i++)
	{
		...
		LONGLONG static lScoreJetton[JETTON_COUNT]={100L,500L,2500L,10000L,50000L,100000L,1000000L};
		...
		//绘画数字
		if (lScoreCount>0L)	DrawNumberString(pDCBuff, lScoreCount/100, m_PointJettonNumber[i].x, m_PointJettonNumber[i].y );
	}
...
int CGameClientView::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	m_btJetton25.Create(NULL,WS_CHILD|WS_VISIBLE|WS_DISABLED,rcCreate,this,IDC_JETTON_BUTTON_25);
	...
	m_btJetton25.SetButtonImage(IDB_BT_JETTON_25,hResInstance,false,false);
	m_btJetton100.SetButtonImage(IDB_BT_JETTON_100,hResInstance,false,false);
	m_btJetton500.SetButtonImage(IDB_BT_JETTON_500,hResInstance,false,false);
...
void CGameClientView::PlaceUserJetton(BYTE cbViewIndex, LONGLONG lScoreCount)
{
	...
	LONGLONG lScoreIndex[JETTON_COUNT]={100L,500L,2500L,10000L,50000L,100000L,1000000L};
...
//绘画数字
void CGameClientView::DrawNumberStringWithSpace(CDC * pDC, CString szNumber, INT nXPos, INT nYPos)
{
...
//绘画整数
void CGameClientView::DrawNumberStringWithSpace(CDC * pDC, CString szNumber, CRect rcRect, INT nFormat,bool bBig)
{
	/* CString strNumber, strTempNum;
	strTempNum.Format(_T("%I64d"), (lNumber>0?lNumber:-lNumber));
...
BOOL CGameClientView::OnSetCursor(CWnd * pWnd, UINT nHitTest, UINT uMessage)
{
	if (m_lCurrentJetton!=0L)
	{
		...
		{
			if (lLeaveScore>=1000000L) SetCurrentJetton(1000000L);
			else if (lLeaveScore>=100000L) SetCurrentJetton(100000L);
			else if (lLeaveScore>=50000L) SetCurrentJetton(50000L);
			else if (lLeaveScore>=10000L) SetCurrentJetton(10000L);
			else if (lLeaveScore>=2500L) SetCurrentJetton(2500L);
		...
		case 2500:
			{
				SetCursor(LoadCursor(AfxGetInstanceHandle(),MAKEINTRESOURCE(IDC_SCORE_25)));
				return TRUE;
			}
		case 10000:
			{
...
void CGameClientView::InsertGameEndInfo()
{
	...
	double dBankCurGameScore = m_lBankerCurGameScore;
	strInfo.Format(TEXT("庄家:%.2lf"),dBankCurGameScore/100);
	InsertAllChatMessage(strInfo,0);

	double dMeCurGameScore = m_lMeCurGameScore;
	strInfo.Format(TEXT("本家:%.2lf"),dMeCurGameScore/100);
	InsertAllChatMessage(strInfo,0);
...
void CGameClientView::ShowGameResult(CDC *pDC, int nWidth, int nHeight)
{
	...
	DrawNumberStringWithSpace(pDC, AddDecimal(m_lMeCurGameScore),rcMeWinScore, DT_END_ELLIPSIS | DT_VCENTER | DT_CENTER | DT_SINGLELINE,true);
	DrawNumberStringWithSpace(pDC, AddDecimal(m_lMeCurGameReturnScore),rcMeReturnScore, DT_END_ELLIPSIS | DT_VCENTER | DT_CENTER | DT_SINGLELINE,true);
	...
	DrawNumberStringWithSpace(pDC, AddDecimal(m_lBankerCurGameScore),rcBankerWinScore, DT_END_ELLIPSIS | DT_VCENTER | DT_CENTER | DT_SINGLELINE,true);
}
...
void CGameClientView::DrawMeJettonNumber(CDC *pDC)
{
	...
	for (int nAreaIndex=1; nAreaIndex<=AREA_COUNT; ++nAreaIndex)
	{
		if (m_lUserJettonScore[nAreaIndex] > 0 )
			DrawNumberString(pDC,m_lUserJettonScore[nAreaIndex]/100,m_PointJettonNumber[nAreaIndex-1].x,m_PointJettonNumber[nAreaIndex-1].y+25, true);
	}
}
...
void CGameClientView::DrawBankerList(CDC *pDC, int nWidth, int nHeight)
{
	...
	for( int i = m_nShowValleyIndex; i < m_ValleysList.GetCount(); ++i )
	{
		...
		DrawNumberStringWithSpace( pDC, AddDecimal( pIClientUserItem->GetUserScore() ) , rectScore, DT_END_ELLIPSIS|DT_LEFT|DT_VCENTER|DT_SINGLELINE);
...
void CGameClientView::DrawBankerInfo(CDC *pDC,int nWidth,int nHeight)
{
	...
	StrRect.right = StrRect.left + 94;
	StrRect.bottom = StrRect.top + 15;
	DrawNumberStringWithSpace(pDC, AddDecimal( m_lBankerWinScore ),StrRect);
	...
	StrRect.bottom = StrRect.top + 15;
	DrawNumberStringWithSpace(pDC, AddComma( m_wBankerTime ),StrRect);
	...
	StrRect.bottom = StrRect.top + 15;
	DrawNumberStringWithSpace(pDC, AddDecimal( pUserData==NULL?0:pUserData->lScore ), StrRect);	
}
...
void CGameClientView::DrawMeInfo(CDC *pDC,int nWidth,int nHeight)
{
	...
	if ( pMeUserData != NULL )
	{
		...
		rcDraw.top=95;
		rcDraw.bottom=rcDraw.top+15;
		DrawNumberStringWithSpace(pDC, AddDecimal( m_lMeStatisticScore ),rcDraw,DT_VCENTER|DT_SINGLELINE|DT_RIGHT|DT_END_ELLIPSIS);
		...
		rcDraw.bottom=rcDraw.top+15;
		DrawNumberStringWithSpace(pDC, AddDecimal( pMeUserData->lScore-lMeJetton ),rcDraw,DT_VCENTER|DT_SINGLELINE|DT_RIGHT|DT_END_ELLIPSIS);
	}
}
