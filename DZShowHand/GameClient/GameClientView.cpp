...
int CGameClientView::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	m_btMin.SetButtonImage( BT_MIN,hResInstance,false, false );
	m_btClose.SetButtonImage( BT_CLOSE, hResInstance, false, false);
...
VOID CGameClientView::DrawGameView(CDC * pDC, INT nWidth, INT nHeight)
{
	...
	for (WORD i=0;i<GAME_PLAYER;i++)
	{
		...
		if (pClientUserItem!=NULL)
		{
			...
			//用户地址
			TCHAR szAddress[128]=TEXT("");
			tagUserInfo * pUserInfo = pClientUserItem->GetUserInfo();
			_sntprintf( szAddress, CountArray(szAddress), TEXT("%s-%s"), pClientUserItem->GetNickName(), pUserInfo->szUserAddress );
			...
			m_ViewFontEx.DrawText(pDC, szAddress, &rcName, RGB(255,255,255),DT_CENTER|DT_VCENTER|DT_SINGLELINE|DT_END_ELLIPSIS);
			...
			m_ViewFontEx.DrawText(pDC,AddDecimal(lTempCount),&rcName1,RGB(255,255,4),DT_CENTER|DT_VCENTER|DT_SINGLELINE|DT_END_ELLIPSIS);
...
void CGameClientView::DrawGameFrame(CDC *pDC, int nWidth, int nHeight)
{
	...
	m_FontCaption.DrawText( pDC, m_StringCaption, rcCaption, RGB(255,255,255), DT_VCENTER|DT_LEFT);
}
