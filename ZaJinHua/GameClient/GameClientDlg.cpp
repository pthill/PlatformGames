...
bool CGameClientEngine::OnSubGameEnd(const void * pBuffer, WORD wDataSize)
{
	...
	for (WORD i=0;i<GAME_PLAYER;i++)
	{
		...
		{
			double dGameScore = pGameEnd->lGameScore[i];
			 _sntprintf(szBuffer,CountArray(szBuffer),TEXT("【%s】:%.2lf"),m_szAccounts[i],dGameScore/100);
...
//	m_GameClientView.m_btAddScore.ShowWindow(false);
	...
	m_GameClientView.m_btJetton_1.SetButtonImage(IDB_BT_JZ_X1,AfxGetInstanceHandle(),false,false);
	m_GameClientView.m_btJetton_3.SetButtonImage(IDB_BT_JZ_X3,AfxGetInstanceHandle(),false,false);
	m_GameClientView.m_btJetton_5.SetButtonImage(IDB_BT_JZ_X5,AfxGetInstanceHandle(),false,false);
...
//		m_GameClientView.m_btAddScore.EnableWindow(((m_lCurrentTimes*m_lCellScore) < m_lMaxCellScore)?FALSE:FALSE);
		if((m_lCurrentTimes*m_lCellScore) < m_lMaxCellScore)
		{
			...
			m_GameClientView.m_btJetton_3.EnableWindow((m_lCurrentTimes<=8)?TRUE:FALSE);
