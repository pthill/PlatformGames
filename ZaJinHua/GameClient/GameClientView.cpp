...
int CGameClientView::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	m_btJetton_3.Create(TEXT(""),WS_CHILD,rcCreate,this,IDC_JETTON_3);
	...
	m_btJetton_1.SetButtonImage(IDB_BT_JZ_X1,hInstance,false,false);
	m_btJetton_3.SetButtonImage(IDB_BT_JZ_X3,hInstance,false,false);
	m_btJetton_5.SetButtonImage(IDB_BT_JZ_X5,hInstance,false,false);
...
VOID CGameClientView::RectifyControl(INT nWidth, INT nHeight)
{
	...
	DeferWindowPos(hDwp,m_btJetton_3,NULL,210+4*79,nHeight-102,0,0,uFlags);
	...
	if(!m_bStopDraw)
	{
		...
		for (WORD i=0;i<GAME_PLAYER;i++)
		{
			...
			if (pClientUserItem!=NULL)
			{
				...
				_sntprintf( szBuffer, sizeof(szBuffer), TEXT("%s"), AddDecimal( lLeaveScore ) );
			
				if(i==MY_SELF_CHAIRID)
				{
					CSize SizeMyselfNum;
					SizeMyselfNum.SetSize(m_PngImageMyself.GetWidth()/14,m_PngImageMyself.GetHeight());
					...
					{
						if(i==2) {
							m_PngImageMyself.DrawImage(pDC,503+SizeMyselfNum.cx*(9-nDecimal-i),nHeight-26,SizeMyselfNum.cx,SizeMyselfNum.cy,(3)*SizeMyselfNum.cx,0,SizeMyselfNum.cx,SizeMyselfNum.cy);
							nDecimal = 1;
						}

						LONGLONG tmp = lLeaveScore%10+nOffset;
						
						// 用户自己的金币
						m_PngImageMyself.DrawImage(pDC,503+SizeMyselfNum.cx*(9-nDecimal-i),nHeight-26,SizeMyselfNum.cx,SizeMyselfNum.cy,tmp*SizeMyselfNum.cx,0,SizeMyselfNum.cx,SizeMyselfNum.cy);
						if((lLeaveScore/=10)==0) break;
					}

					//补小数点
					if(i<2) {
						m_PngImageMyself.DrawImage(pDC,503+SizeMyselfNum.cx*(9-i-1),nHeight-26,SizeMyselfNum.cx,SizeMyselfNum.cy,(3)*SizeMyselfNum.cx,0,SizeMyselfNum.cx,SizeMyselfNum.cy);
						m_PngImageMyself.DrawImage(pDC,503+SizeMyselfNum.cx*(9-i-2),nHeight-26,SizeMyselfNum.cx,SizeMyselfNum.cy,(4)*SizeMyselfNum.cx,0,SizeMyselfNum.cx,SizeMyselfNum.cy);
					}
				}
				else
				{
		...
		{	
			...
			//桌上总注值
			CSize SizeXZNumber;
			SizeXZNumber.SetSize(m_PngaImageXZNumber.GetWidth()/14,m_PngaImageXZNumber.GetHeight());
			...
			{
				if(i==2) {
					m_PngaImageXZNumber.DrawImage(pDC,nWidth/2-61+(10-nDecimal-i)*SizeXZNumber.cx,nHeight-500,SizeXZNumber.cx,SizeXZNumber.cy,(3)*SizeXZNumber.cx,0,SizeXZNumber.cx,SizeXZNumber.cy);
					nDecimal = 1;
				}
				LONGLONG wXZTmp = lTableScore%10+nOffset;
				m_PngaImageXZNumber.DrawImage(pDC,nWidth/2-61+(10-nDecimal-i)*SizeXZNumber.cx,nHeight-500,SizeXZNumber.cx,SizeXZNumber.cy,wXZTmp*SizeXZNumber.cx,0,SizeXZNumber.cx,SizeXZNumber.cy);
		...
		for (BYTE i=0;i<GAME_PLAYER;i++)
		{
			...
			//绘画数目
			TCHAR szBuffer[64];
			_sntprintf(szBuffer,CountArray(szBuffer),TEXT("%s"), AddDecimal(m_lTableScore[i]) );
		...
		{
			CSize SizeFDNum;
			SizeFDNum.SetSize(m_PngImageFDNumber.GetWidth()/14,m_PngImageFDNumber.GetHeight());
			...
			{
				if(i==2) {
					m_PngImageFDNumber.DrawImage(pDC,nXDanzhuPos+(wDanzhu-nDecimal-i)*SizeFDNum.cx,78-30,SizeFDNum.cx,SizeFDNum.cy,(3)*SizeFDNum.cx,0,SizeFDNum.cx,SizeFDNum.cy);
					nDecimal = 1;
				}
				DWORD wTmp = lDanzhu%10;
				lDanzhu/=10;
				m_PngImageFDNumber.DrawImage(pDC,nXDanzhuPos+(wDanzhu-nDecimal-i)*SizeFDNum.cx,78-30,SizeFDNum.cx,SizeFDNum.cy,(wTmp+nOffset)*SizeFDNum.cx,0,SizeFDNum.cx,SizeFDNum.cy);
			}

			//补小数点
			if(i<2) {
				m_PngImageFDNumber.DrawImage(pDC,nXDanzhuPos+(wDanzhu-i-1)*SizeFDNum.cx,78-30,SizeFDNum.cx,SizeFDNum.cy,(3)*SizeFDNum.cx,0,SizeFDNum.cx,SizeFDNum.cy);
				m_PngImageFDNumber.DrawImage(pDC,nXDanzhuPos+(wDanzhu-i-2)*SizeFDNum.cx,78-30,SizeFDNum.cx,SizeFDNum.cy,(4)*SizeFDNum.cx,0,SizeFDNum.cx,SizeFDNum.cy);
			}

			//封顶
			nDecimal = 0;
			for( i = 0 ; i < wFengding ; ++i)
			{
				if(i==2) {
					m_PngImageFDNumber.DrawImage(pDC,nXFengdingPos+(wFengding-nDecimal-i)*SizeFDNum.cx,52-30,SizeFDNum.cx,SizeFDNum.cy,(3)*SizeFDNum.cx,0,SizeFDNum.cx,SizeFDNum.cy);
					nDecimal = 1;
				}
				DWORD wTmp = lFending%10;
				lFending/=10;
				m_PngImageFDNumber.DrawImage(pDC,nXFengdingPos+(wFengding-nDecimal-i)*SizeFDNum.cx,52-30,SizeFDNum.cx,SizeFDNum.cy,(wTmp+nOffset)*SizeFDNum.cx,0,SizeFDNum.cx,SizeFDNum.cy);
			}
			
			//补小数点
			if(i<2) {
				m_PngImageFDNumber.DrawImage(pDC,nXFengdingPos+(wFengding-i-1)*SizeFDNum.cx,52-30,SizeFDNum.cx,SizeFDNum.cy,(3)*SizeFDNum.cx,0,SizeFDNum.cx,SizeFDNum.cy);
				m_PngImageFDNumber.DrawImage(pDC,nXFengdingPos+(wFengding-i-2)*SizeFDNum.cx,52-30,SizeFDNum.cx,SizeFDNum.cy,(4)*SizeFDNum.cx,0,SizeFDNum.cx,SizeFDNum.cy);
			}
