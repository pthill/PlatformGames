...
VOID CNumberControl::DrawNumberControl( CDC *pDC )
{
	...
	m_PngScoreBack.DrawImage(pDC,nXPos,nYPos,nScoreBackWidth,nScoreBackHeight, (m_lScore<0L)?nScoreBackWidth:0,0);
	nXPos += nScoreBackWidth;

	INT nNumWidth = m_PngScoreNum.GetWidth()/14;
	...
	for( INT_PTR i = m_arScoreNum.GetCount()-1; i >= 0; i-- )
	{
		...
		//加小数点
		if( i==1 ) {
			m_PngScoreBack.DrawImage(pDC,nXPos,nYPos,nScoreBackWidth,nScoreBackHeight,nScoreBackWidth*(m_lScore<0L?3:2),0);

			m_PngScoreNum.DrawImage( pDC,nXPos, nYPos+4, nNumWidth, nNumHeight, (3)*nNumWidth, 0 );
		...
		else
		{
			...
			BYTE byCellNum = m_arScoreNum[i]+4;
